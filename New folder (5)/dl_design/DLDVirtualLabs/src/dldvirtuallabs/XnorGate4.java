...
public class XnorGate4 extends Element {
     ...
     */
    XnorGate4(int id, String type, int inpId, int outId, Point coord) {
        elementID = id;
        elementType = type;
    ...
    public void draw(Graphics g, Point p){
        ...
        /* Define four curves for XNOR Gate */
        ...
        QuadCurve2D q4 = new QuadCurve2D.Float(p.x-37, p.y-75, p.x-7, p.y, p.x-37, p.y+75);
        ...
        /* Draw the extra q4 curve that symbolizez XNOR gate */
        g2d.draw(q4);

        /* Draw the circle in the front XNOR to denote the NOT of XOR */
        g2d.fillOval(p.x+35-7, p.y-7, 14, 14);

        inputList.elementAt(0).draw(g, new Point(p.x-27, p.y-60));
        inputList.elementAt(1).draw(g, new Point(p.x-27, p.y-20));
        inputList.elementAt(2).draw(g, new Point(p.x-27, p.y+20));
        inputList.elementAt(3).draw(g, new Point(p.x-27, p.y+60));
